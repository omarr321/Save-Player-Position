...
public class CommandSpp implements CommandExecutor {
        ...
        switch(args[0]) {
            ...
            case "command":
                ...
                if (args.length == 3) {
                    switch (args[1]) {
                        case "add":
                            configManager.addCommand(args[2], player);
                            break;
                        case "remove":
                            configManager.removeCommand(args[2], player);
                            break;
                        default:
                            sppMessager.sendMessage(player, "Error: Unknown command!", ChatColor.RED);
                    }
    ...
    private void showHelp(Player player) {
        ...
        sppMessager.sendMessage(player, "/spp command [add/remove] \\<command\\>",ChatColor.GREEN, "- Adds/Removes a command to the whitelist", ChatColor.RESET);
        sppMessager.sendMessage(player, "/spp command list",ChatColor.GREEN, "- Lists all the command in the whitelist", ChatColor.RESET);
